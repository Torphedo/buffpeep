...
void update_camera(mat4* view) {
  ...
  camera_orbit_angles.y = clampf(camera_orbit_angles.y, glm_rad(89.9999) * -1.0f, glm_rad(89.9999));

  // Get XYZ positions using trig on our angles
  camera_pos.x = sin(camera_orbit_angles.x) * radius;
  camera_pos.z = cos(camera_orbit_angles.x) * radius;
  camera_pos.y = sin(camera_orbit_angles.y) * radius;
  ...
  float z_diff = (1.0f - cos(camera_orbit_angles.y)) * radius;
